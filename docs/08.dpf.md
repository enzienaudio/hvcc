...
Heavy can generate a LV2 and VST2 plugins from your patch using the [Distrho Plugin Format](https://github.com/DISTRHO/DPF). It can be either a synth (output-only) or an effect (input and output), supports an arbitrary number of parameters, and can process midi events.
