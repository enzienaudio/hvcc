...
The Heavy C++ API is equivalent to the [C API](10.c.md). Its usage is a matter of preference, but may make the organisation of systems with multiple context types and instances easier.

The pure virtual interface class to all Heavy contexts is called `HeavyContextInterface` and is defined in [HeavyContextInterface.hpp](https://github.com/Wasted-Audio/heavy/blob/master/src/HeavyContextInterface.hpp). The patch-specific header is `Heavy_example.hpp`.
